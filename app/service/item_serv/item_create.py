...
from app.service.item_serv.item_checks import check_duplicate_name, check_parent_exists
...
def _upload_file_to_storage(
    filedata: UploadFile, file: Item, storage: StorageClientInterface
):
    try:
        storage.save(
            drive_bucketid,
...
def _create_folders(
    db: Session, ownerid: str, parentid: str | None, folders: list[Item]
) -> str | None:
...
def item_save_item_serv(
    db: Session, filedata: UploadFile, ownerid: str, parentid: str | None
) -> Item:

    check_parent_exists(db, ownerid, parentid)

    folders, file = _create_items_from_path(
        filedata,
        ownerid,
    )

    _upload_file_to_storage(filedata, file, storage)
    crr_parentid = _create_folders(db, ownerid, parentid, folders)
...
def item_save_folder_serv(db: Session, ownerid: str, name: str, parentid: str | None):
    check_duplicate_name(db, ownerid, parentid, name, ItemType.FOLDER.value)
    check_parent_exists(db, ownerid, parentid)
